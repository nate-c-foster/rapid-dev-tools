...
def getAlarmReport(rootTagPath):
	...
	for alarmTag in alarmTags:
		...
		if system.tag.readBlocking(alarmPath + '.enabled')[0].value and system.tag.readBlocking(alarmPath + '/Alarm.enabled')[0].value:
			...
			alarmValueSource, alarmValueProperty, alarmValueQuality, alarmValue = dataset.util.getTagProperties(alarmPath+'/Alarm')
			...
			enableValueSource, enabledValueProperty, enableValueQuality, enableValue = dataset.util.getTagProperties(alarmPath+'/Enable')
			setpointValueSource, setpointValueProperty, setpointValueQuality, setpointValue = dataset.util.getTagProperties(alarmPath+'/Setpoint')
			delayValueSource, delayValueProperty, delayValueQuality, delayValue = dataset.util.getTagProperties(alarmPath+'/Delay')
